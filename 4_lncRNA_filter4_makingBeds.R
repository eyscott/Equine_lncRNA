...
refined_bed <- read.table("allTissues_BED/refined.bed", header=F, stringsAsFactors=F)
refined <- refined_bed[ ,c("V1","V2","V3","V4")]
setwd("~/Dropbox/lncRNA")
f3 <- read.table("lncRNA_f3_IDs", header=F, stringsAsFactors=F)
#make sure the mergedtrans has none of the novel I,II,III lncRNA
library(dplyr)
refined_nolncRNA <- anti_join(refined,f3)
refined_nolncRNA_bed <- anti_join(refined_bed,f3)
#order chr properly
...
setwd("~/Dropbox/lncRNA")
refined_nolncRNA_bed <- read.table("refined_nolncRNA.bed", header=F,stringsAsFactors=F )
...
refined_nolncRNA_bed_5$minus[refined_nolncRNA_5$minus < 0] <- 0
#order chr properly
